...
from .timing import get_time_us
...
class STune:
    ...
    def configure(self, 
        ...
        self._buffer_size = max(1, int(self._samples * 0.06))  # 6% of samples for buffer
        ...
        if self._serial_mode != SerialMode.PRINT_OFF:
            print(f"STune configured: {samples} samples over {test_time_sec}s, "
    ...
    def _run_test_phase(self, us_now: float, us_elapsed: float, settle_elapsed: float) -> TunerStatus:
        ...
        if self.pv_inst > self.e_stop and not self.e_stop_abort:
            ...
            raise TuningError("Emergency stop triggered: process variable exceeded limit")
        ...
        if settle_elapsed >= self._settle_period_us:
            ...
            if us_elapsed >= self._sample_period_us and not self.e_stop_abort:
                ...
                if self._serial_mode in [SerialMode.PRINT_ALL, SerialMode.PRINT_DEBUG]:
                    ...
                    print(f" sec: {remaining_time:.4f}  out: {self._output_start}  "
    ...
    def _process_sample(self, us_now: float) -> TunerStatus:
        ...
        if (pv_inst_resolution > self.EPSILON and 
            pv_inst_resolution < self.pv_inst_res):
            self.pv_inst_res = pv_inst_resolution
            
        if (pv_avg_resolution > self.EPSILON and 
            pv_avg_resolution < self.pv_avg_res):
        ...
        if self.sample_count == 0:
            ...
            self.pv_inst_res = abs(self.pv_inst) if abs(self.pv_inst) > self.EPSILON else 1.0
            self.pv_avg_res = abs(self.pv_inst) if abs(self.pv_inst) > self.EPSILON else 1.0
    ...
    def _detect_inflection_point(self) -> None:
        ...
        if self.ip_count == ip_threshold:
            ...
            self._tu = (((self.pv_max - self.pv_start) / self.slope_ip) * 
                       self._tangent_period_us * 1e-6) - self._td
    ...
    def set_input_output(self, 
                        ...
                        output_var: Callable[[float], None]) -> None:
    ...
    def _print_results(self) -> None:
        ...
        if self._serial_mode in [SerialMode.PRINT_ALL, SerialMode.PRINT_DEBUG, SerialMode.PRINT_SUMMARY]:
            print()
            ...
            print(f" Controller TunerAction: {action_names.get(self._action, 'Unknown')}")
            print()
            ...
            if (self._serial_mode == SerialMode.PRINT_DEBUG and 
                self._action in [TunerAction.DIRECT_IP, TunerAction.REVERSE_IP]):
                print(f" Ip Sec:            {self.ip_us * 1e-6:.4f}")
                direction = "↑" if self._action in [TunerAction.DIRECT_IP, TunerAction.DIRECT_5T] else "↓"
    ...
    def _print_test_run(self) -> None:
        ...
        if (self.sample_count < self._samples and 
            self._serial_mode in [SerialMode.PRINT_ALL, SerialMode.PRINT_DEBUG]):
            
            print(f" sec: {self.us * 1e-6:.4f}", end="")
            print(f"  out: {self._output_var.__name__ if hasattr(self._output_var, '__name__') else 'output'}", end="")
            print(f"  pv: {self.pv_inst:.3f}", end="")
            
            if (self._serial_mode == SerialMode.PRINT_DEBUG and 
                self._action in [TunerAction.DIRECT_5T, TunerAction.REVERSE_5T]):
    ...
    def plotter(self, input_val: float, output_val: float, setpoint: float, 
                output_scale: float = 1.0, every_nth: int = 1) -> None:
        """Print plotter-formatted data for visualization."""
        if self.plot_count >= every_nth:
            self.plot_count = 1
            print(f"Setpoint:{setpoint}, Input:{input_val}, Output:{output_val * output_scale},")
        else:
            self.plot_count += 1