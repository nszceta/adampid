...
class AutoAdamPID:
    ...
    def __init__(
        ...
        if self._should_auto_tune():
            logger.info("Auto-tuning enabled and required - will start when input/setpoint provided")
        else:
    ...
    def set_input(self, input_value: float) -> None:
        ...
        # Pass to tuner if tuning is in progress
    ...
    def compute(self) -> bool:
        ...
        # Start auto-tuning if needed and both input/setpoint are available
        if self._should_auto_tune() and not self._tuning_in_progress and not self._auto_tuning_complete:
            if self._setpoint_valid:  # Need setpoint to know where we're going
                logger.info("Starting auto-tuning process")
                self._start_auto_tuning()
            else:
                logger.warning("Setpoint must be set before auto-tuning can begin")
                return False
    ...
    def _complete_auto_tuning(self) -> None:
        ...
        try:
            ...
            logger.info("Auto-tuning process completed successfully and configuration saved")
    ...
    def _validate_config(self) -> None:
        ...
        for section in required_sections:
            if section not in self.config:
                raise AutoAdamPIDError(f"Missing required configuration section: {section}")
