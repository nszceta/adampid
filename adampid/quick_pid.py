...
class AdamPID:
    ...
    def __init__(self, 
        ...
        # Timing
        self._sample_time_us: int = 100_000  # 0.1 second default
        ...
        self._last_time = get_time_us() - self._sample_time_us
    ...
    def _apply_conditional_anti_windup(self, p_error_term: float, p_meas_term: float, d_error: float) -> None:
        """
        ...
        """
        aw = False
        i_term_out = (p_error_term - p_meas_term) + self._ki * (self._i_term + self._error)
    ...
    def set_output_limits(self, min_output: float, max_output: float) -> None:
        ...
        if self._mode != Control.MANUAL and self._output_var:
            ...
            if current_output is not None:
                constrained_output = self._constrain(current_output, min_output, max_output)
    ...
    def set_mode(self, mode: Control) -> None:
        ...
        if mode == Control.TOGGLE:
            self._mode = Control.AUTOMATIC if self._mode == Control.MANUAL else Control.MANUAL
        else:
            self._mode = mode
    ...
    def initialize(self) -> None:
        ...
        current_output = self._get_current_output()
        if current_output is not None:
            self.output_sum = self._constrain(current_output, self._out_min, self._out_max)
        else:
            self.output_sum = 0.0
    ...
    def __repr__(self) -> str:
        ...
        return (f"AdamPID(Kp={self._disp_kp:.3f}, Ki={self._disp_ki:.3f}, "
